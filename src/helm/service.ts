...
type SyncHelmTemplateCacheFailure = {
  chartName: string;
  releaseName: string;
  message: string;
};

type SyncHelmTemplateCacheOptions = {
  onCacheChanged: () => void;
...
export class HelmService {
  ...
  async syncHelmTemplateCache({
    onCacheChanged,
    workspacePath,
    config,
  }: SyncHelmTemplateCacheOptions): Promise<void> {
    ...
    await vscode.window.withProgress(
      ...
      async (progress) => {
        ...
        const failures: Array<SyncHelmTemplateCacheFailure | undefined> = await Promise.all(
          renderTargets.map(async (target) => {
            ...
            try {
              ...
              return {
                ...
                message,
              } satisfies SyncHelmTemplateCacheFailure;
            } finally {
              completedCount += 1;
              progress.report({
        ...
        const failedReleases = failures.filter(
          (failure): failure is SyncHelmTemplateCacheFailure => failure !== undefined,
        );
